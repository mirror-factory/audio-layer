...
const INDEX_PATHS = [
  join(ROOT, '.ai-starter', 'research', 'index.json'),
  join(ROOT, '.codex', 'research', 'index.json'),
  join(ROOT, '.claude', 'research', 'index.json'),
];
...
function renderSection(index: ResearchIndex): string {
  ...
  lines.push('Before editing code that touches these libraries, read the corresponding `.ai-starter/research/libraries/*/CURRENT.md` file. If STALE, run `pnpm research:refresh <id>` first.');
  lines.push('');
...
function main(): void {
  const indexPath = INDEX_PATHS.find(path => existsSync(path));
  if (!indexPath) { console.error(`${INDEX_PATHS[0]} not found. Run research-bootstrap.ts first.`); process.exit(1); }
  if (!existsSync(AGENTS_PATH)) { console.error(`${AGENTS_PATH} not found`); process.exit(1); }

  const index = JSON.parse(readFileSync(indexPath, 'utf-8')) as ResearchIndex;
